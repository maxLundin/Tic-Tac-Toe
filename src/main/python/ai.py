...
class AI(Player):
    ...
    def turn(self, game):
        ...
        for key in self.points_tally:
            valid = len(game.remaining) == game.length**2
            for i in range(game.position_dict[key][0] - 1, game.position_dict[key][0] + 2):
                for j in range(game.position_dict[key][1] - 1, game.position_dict[key][1] + 2):
                    if 0 <= i < game.length and 0 <= j < game.length and (game.board[i][j] == 'O' or game.board[i][j] == 'X'):
                        valid = True
            if (self.points_tally[key] > highest_val or highest_pos == 0) and valid and key in game.remaining:
                highest_val = max(0, self.points_tally[key])
                highest_pos = key
